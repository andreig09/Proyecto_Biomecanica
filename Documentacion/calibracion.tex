...
\begin{figure}[ht!]
        \hspace{-1cm}
        \subfloat[Blender]{\includegraphics[trim = 0mm 0mm 0mm 0mm, clip,scale=0.24]{img/calibracion/camaras_blender.png}\label{fig: camaras blender}}
                \hspace{0.01cm}                
...
En la Figura \ref{fig: error reproyeccion} se muestra el promedio y la desviación del error de re-proyección estándar obtenido para cada una de las cámaras.
...
\begin{figure}[ht!]
        \hspace{-1cm}
        \subfloat[Damero en el entorno Blender]{\includegraphics[scale=0.2]{img/calibracion/tablero_blender.png}\label{fig:tablero_blender}}
                \hspace{0.01cm}                
      ...
      \caption{Simulación del damero en Blender y calibración estéreo de un par de cámaras adyacentes}
      \label{fig: configuracion camaras}      
\end{figure}
...
De los dos toolbox vistos anteriormente se obtuvieron las coordenadas intrínsecas y extrínsecas de las cámaras. En el \textit{Multi-Camera Self-Calibration Toolbox} las parámetros extrínsecos de las cámaras son referidos a un sistema de coordenadas desconocido que se establece en el proceso de calibración del toolbox. El origen de coordenadas de ese sistema se ubica en el centroide de todas las posiciones capturadas de la fuente de luz. En el \textit{amcctoolbox} por otra parte, los parámetros extrínsecos de las cámaras son referidos al sistema de coordenadas de la cámara que se define como origen del sistema de coordenadas. En algunos casos puede desearse que el sistema de coordenadas del espacio 3D sea definido por el usuario. En nuestro caso puede quererse que el sistema de coordenadas de la calibración coincida con el sistema de coordenadas 
de Blender.\\

Una opción para realizar esto es colocar en el espacio marcadores en posiciones conocidas. Como se conocen las matrices de proyección referidas a un sistema de coordenadas, por ejemplo $S_1$, se tiene para cada cámara $i$, las matrices de proyección  $P_i|_{S_1}$ referidas a dichos sistema. Se quiere hallar entonces las matrices de proyección referidas a otro sistema, por ejemplo, el sistema $S_2$. Por tanto se buscan las matrices  de proyección $P_i|_{S_2}$ referidas al sistema $S_2$.


Este método presenta la desventaja de que los parámetros extrínsecos de las cámaras se vinculan en relación a sus adyacentes por lo que debe definirse a una de las cámaras como el origen del sistema de coordenadas del espacio respecto a la cual se halla la posición del resto de las cámaras. Por lo tanto el error se acumula  incrementándose para aquellas cámaras que están más alejadas de la que es determinada como el origen del sistema. \\

El toolbox por otra parte, requiere para la calibración estéreo de un par cámaras, establecer en forma automática una correspondencia entre la imagen del damero detectado en la cámara izquierda con la imagen capturada por la cámara derecha. Esto en algunos casos no se logra y la correspondencia entre dichas imagenes debe realizarse forma manual. 
