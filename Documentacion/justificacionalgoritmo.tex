...
En primer lugar, se efectúa la reconstrucción de marcadores utilizando en cada paso métodos con menor precisión y exigencia como la reconstrucción binocular y monocular, donde se usan dos o una cámara respectivamente, en lugar de tres cámaras (reconstrucción trinocular) como en la triangulación estéreo que se describe en la sección \ref{reconstruccion}.

 Si en la salida de cada uno de los bloques anteriores la condición aún no se cumple se pasa al siguiente, donde se asocian los marcadores 3D reconstruidos que aún no fueron identificados con aquellas articulaciones del modelo de esqueleto ajustado en la inicialización, que no tienen ningún marcador asociado. Para esto, se evalúan las distancias de los marcadores no identificados con la posición de las articulaciones del modelo en frames anteriores y se asocian aquellos marcadores que se encuentren a menor distancia a cada una de dichas articulaciones. Al tiempo que se realiza esto, se verifica que la distancia entre marcadores asociados a un mismo hueso del esqueleto se mantenga aproximadamente constante.
 
Por último si aún faltan marcadores sin reconstruirse, se estima su posición evaluando la aceleración del marcador en frames anteriores y verificando que dicha estimación sea coherente con el modelo de esqueleto.


%%EXPLICAR QUE POR TEMAS TEMPORALES NO SE PUDO IMPLEMENTAR ESTO EXACTAMENTE Y EXPLICAR BIEN QUE FUE LO QUE SE IMPLEMENTÓ Y DECIR QUE PARA CADA BLOQUE SE EXPLICA EN UN CAPITULO