...
Hasta ahora todas las pruebas realizadas para los bloques presentados en la sección~\ref{sec:implementacion_bloques_sistema} , fueron realizadas sobre el conjunto entero de cámaras presentadas en la sección (??? REFERENCIA BASE DE DATOS), con 17 puntos de vistas distintos en el espacio de captura de la figura \ref{img_espacio_capura}. Las pruebas de rendimiento con conjuntos reducidos de cámaras son realizadas sobre los últimos dos bloques de reconstrucción y tracking, debido a que no afectan el error en el bloque de segmentación, solo afecta la cantidad de videos para segmentar.
...
\end{tabular}
\caption{Distintas Combinaciones de Amaras del laboratorio sintético, numeradas según la figura \ref{img_espacio_capura} presentada en (??? REFERENCIA DE BASE DE DATOS)}
\end{table}
...
Con la maxima cantidad de cámaras, todos los marcadores son reconstruidos y sus trayectorias seguidas a lo largo de la secuencia, con las medidas de error ya encontradas hasta el momento, de error de marcador menor al centimentro, y errores maximos puntuales del orden de los pocos centimetros. Si se reduce el conjunto de cámaras, para tener una "visión doble" desde cada una de las esquinas en el espacio de captura, se mantiene el buen rendimiento del sistema, con errores similares a los obtenidos anteriormente,    obteniendo leves mejoras en algunos marcadores pero tambien leves incrementos en el error para otros.  
...
Reduciendo el conjunto a 6 cámaras, 
...
\begin{figure}[H]
  \centering
   \subfloat[Trayectorias Movimiento Libre.]{\includegraphics[scale=0.45]{img/Performance/00_Trayectorias_Tracking.png}\label{tracking_movimiento_libre}}\hspace{3 mm}
   \subfloat[Trayectoria Marcador, y Esqueleto reconstruido.]{\includegraphics[scale=0.45]{img/Performance/00_Animacion_Tracking.png}}  
