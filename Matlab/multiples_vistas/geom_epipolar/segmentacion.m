...
for k=1:n_cams %hacer con cada elemento en list_vid
    ...
    if status~=0 %en este caso se obtuvo un error en command 
        ...
        error('system:ComandoFallido','El comando ''%s'' ha devuelto una se̱al de error', command)         
        ...
        if status_mov==0 %en este caso se obtuvo un error al mover  *.xml (OBSERVAR QUE MATLAB DEVUELVE 0 EN CASO DE ERROR)
            ...
            disp(message)
            disp(messageid)
            error('system:ComandoFallido','El comando para mover *.xml a su carpeta correspondiente ha devuelto una se̱al de error')   
        else 
