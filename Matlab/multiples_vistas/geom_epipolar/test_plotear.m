...
%Estructura cam
   ...
   %plotear(cam, n_cam, list_marker, last_frame, n_prev, t_label) %se plotean trayectorias 2D con etiquetas nombre (t_label=0) o numero (t_label=1)

   animacion=1;
   if animacion==1
       for i=1:last_frame
           if (i-n_prev)<1
               plotear(cam, n_cam, list_marker, i, i-1, t_label) %se plotean trayectorias 2D con etiquetas nombre (t_label=0) o numero (t_label=1)
               pause(0.1)
           else
               plotear(cam, n_cam, list_marker, i, n_prev, t_label) %se plotean trayectorias 2D con etiquetas nombre (t_label=0) o numero (t_label=1)
               pause(0.1)
           end
       end
   end