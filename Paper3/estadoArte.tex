...
Respecto a la búsqueda de bibliografía se han encontrado diversos artículos que hacen una revisión de las distintas técnicas de captura de movimiento. Dado que nuestra búsqueda estaba centrada en el desarrollo de un sistema de buena precisión pero que además fuera accesible, orientamos la búsqueda hacia sistemas que emplean cámaras ópticas y marcadores, sin embargo no se han encontrado artículos que resumieran el estado del arte de los sistemas de captura de estas características. En cambio si se han encontrado diversos artículos que describen implementaciones , totales o parciales que utilizan esta metodología de captura. Dicha bibliografía permitió conocer los procesos principales con los que debe contar un sistema de estas características que en general son \emph{Adquisición}, \emph{Calibración}, \emph{Detección de marcadores}, \emph{Reconstrucción} y \emph{Seguimiento}. Para las procesos de \emph{Reconstrucción} y \emph{Seguimiento} se encontró un Tesis elaborada por Herda. Dicha tesis fue elegida como base para el desarrollo del sistema. Esta elección se basa en que el sistema propuesto posee las características buscadas y además es citada en varios artículos. Sin embargo esta tesis  presenta ciertas ambigüedades respecto a la descripción de la metodología utilizada en algunos procesos.
...
También se analizaron las soluciones que se encuentran actualmente disponibles.  Por un lado se tienen los sistemas de captura de movimiento comerciales que destacan por su performance. Algunos de estos sistemas son: Vicon , Qualysis, Optitrack, Massive, Motion analysis, PhaseSpace. La mayoría de estos sistemas utilizan cámaras infrarrojas, lo que permite una detección más eficaz de marcadores. Sin embargo estos sistemas presentan la desventaja de su elevado costo. Se han analizado también las alternativas de software libre. En este caso no se han encontrado implementaciones que realicen todas las etapas de un sistema de captura de movimiento en forma íntegra. Uno de los sistemas analizados es el Kinovea el cual está orientado al deporte y que permite un análisis del movimiento en dos dimensiones con un enfoque esencialmente cualitativo, y que por tanto no cumple no los requerimientos necesarios. Sin embargo sí existen implementaciones que permiten realizar alguna etapa de un sistema de captura toolbox en Matlab o librerías como OpenCV para el tratamiento de imágenes. Alguna de estas herramientas fueron utilizadas en el sistema desarrollado.

Por otra parte se realizó una búsqueda bases de datos con videos de capturas de movimiento que permitieran realizar tests de performance sobre el sistema a desarrollar. Si bien se encontraron numerosas bases de datos, las mismas fueron descartadas por no ajustarse completamente  a las hipótesis que se plantearon en este trabajo. En particular dichas bases fueron diseñadas para una captura de movimiento con camaras infrarrojas y no convencionales. (mejorar esta parte).Por otra parte presentaban además  inadecuada cantidad o posición de las cámaras, condiciones del laboratorio que dicultan y en algunos casos imposibilitan, una correcta segmentación a partir de la información óptica. Y por último la ausencia o tamaño inadecuado de los marcadores, son los factores más importantes. De todas maneras cabe destacar que se genera un relevamiento de bases de datos para el movimiento humano y se profundiza en las características usuales presentes en dichas bases de datos, logrando obtener un conjunto de conceptos y herramientas importantes para el proyecto. Previo análisis de los parámetros involucrados, se enumeran consideraciones a tener en cuenta a la hora de generar un laboratorio de captura. (tambien se utilizaron como ground truth).
