...
m_dist_s2 = m_markers(1:end-1) * m_markers(2:end)'

if m_dist_s2 == 0   %----------condicion agregada para solucionar el que no hayan marcadores de detectados en una cámara
    Xrec = [];  % si la cantidad de combinaciones posibles de rectas es cero, no se puede recontruir nada
else
...
for i=v_cams(1:end-1)
    cont = cont +1;
    j = v_cams(cont+1);
    
            if n_markers_cam{i} > 0 &&  n_markers_cam{j} > 0 %---- condicion agregada para los casos en que no hay marcadores detectados en una de las camaras
...
l_m_dist = m_dist(6,:) < inf;
m_dist = m_dist(:,l_m_dist);
...
%tot_markers = min (m_markers); % se modifica tot_markers. No tiene sentido que se reconstruyan 13 marcadores sino el maximo detectado en una de las camaras, ese es el maximo de marcadores que son posibles reconstruir
...
if m_dist_valid(5,1) < umbral_dist
    

        [X, ~, ~, ~, ~]=validation3D_fast2(cam, cam_i, cam_d, x_cam, P, valid_points, 'index', ind_i, ind_d, 'umbral', umbral );
